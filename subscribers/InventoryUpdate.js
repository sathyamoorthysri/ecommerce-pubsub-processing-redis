...
const subscribeToOrderEvents = async () => {
    try {
        ...
        await redisClient.subscribe('order_events', (message) => {
            ...
            console.log(`------------------------------------------------------------------------------------------`);
            console.log(`[${new Date().toISOString()}] | Step 1 : Inventory Update request receieved through subscriber from Redis:`);
...
const processOrder = async (eventData) => {
    try {
        ...
        console.log(`[${new Date().toISOString()}] | Step 2 : Update:  Inventory Update triggered ${eventData.orderId} for user ${eventData.userId}`);
        
        // Simulate order processing logic here
        await delayWithSpinner(2000);        

        console.log(`[${new Date().toISOString()}] | Step 3 : Update:  Inventory Update completed successfully ${eventData.orderId}, user ${eventData.userId}`);
