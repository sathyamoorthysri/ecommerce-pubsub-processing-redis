...
const subscribeToOrderEvents = async () => {
    try {
        ...
        await redisClient.subscribe('order_events', (message) => {
            ...
            console.log(`------------------------------------------------------------------------------------------`);
            console.log(`[${new Date().toISOString()}] | Step 1 : Email Confirmation Update request receieved through subscriber from Redis:`);
...
const processOrder = async (eventData) => {
    try {
        ...
        console.log(`[${new Date().toISOString()}] | Step 2 : Update:  Email Confirmation messaging triggered ${eventData.orderId} for user ${eventData.userId}`);
        
        // Simulate order processing logic here
        await delayWithSpinner(2000);        

        console.log(`[${new Date().toISOString()}] | Step 3 : Update:  Email Confirmation message sent successfully ${eventData.orderId}, user ${eventData.userId}`);
